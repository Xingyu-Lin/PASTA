...
from utils import AverageValueMeter, apply_random_rotation, save, resume, visualize_point_clouds
...
def main_worker(gpu, save_dir, ngpus_per_node, args):
    ...
    if args.log_name is not None:
        log_dir = "data/PointFlow/%s/runs" % args.log_name
    else:
        log_dir = "data/PointFlow/time-%d/runs/" % time.time()
    ...
    if not args.distributed or (args.rank % ngpus_per_node == 0):
        variant_path = os.path.join(save_dir, 'variant.json')
        import json
    ...
    if (not args.distributed or (args.rank % ngpus_per_node == 0)) and args.dataset_type != 'dynabs':
        ...
        stat_path = os.path.join(save_dir, 'train_stat.pkl')
        with open(stat_path, 'wb') as f:
                pickle.dump(tr_dataset.stat, f)
        stat_path = os.path.join(save_dir, 'eval_stat.pkl')
        with open(stat_path, 'wb') as f:
                pickle.dump(te_dataset.stat, f)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if (epoch + 1) % args.exp_decay_freq == 0 and epoch > 0:
        ...
        if (epoch + 1) % args.viz_freq == 0:
            ...
            for idx in range(min(10, inputs.size(0))):
                res = visualize_point_clouds(samples[idx], inputs[idx], idx, args.dataset_type,
            ...
            if args.use_latent_flow:
                ...
                for idx in range(num_samples):
                    ...
                    results.append(res)
                res = np.concatenate(results, axis=1)
                imageio.imwrite(os.path.join(save_dir, 'images', 'tr_vis_unconditioned_epoch%d-gpu%s.png' % (epoch, args.gpu)),
                                res.transpose((1, 2, 0)))
                if writer is not None:
...
def main():
    ...
    save_dir = os.path.join("data/PointFlow", args.log_name)

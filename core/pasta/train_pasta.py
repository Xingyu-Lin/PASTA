...
from core.utils.core_utils import set_ipdb_debugger
...
def run_task(arg_vv, log_dir, exp_name):  # Chester launch
    ...
    from core.pasta.agent import SetAgent
    from core.utils.diffskill_utils import aggregate_traj_info, dict_add_prefix
    from core.eval.eval_helper import eval_skills, eval_vae, eval_plan

    device = 'cuda'
    # # ----------preparation done------------------
    agent = SetAgent(args, None, num_tools=args.num_tools, device=device)
    print('SetAgent created')
    ...
    if args.run_plan:
        ...
        epoch = 0
        # Plan
        plan_info = eval_plan(args, env, agent, epoch, demo=False, profile=True)
    ...
    agent.vae.generate_cached_set_buffer(buffer)
    ...
    for epoch in range(args.il_num_epoch):
        ...
        if epoch % args.il_eval_freq == 0:
            ...
            if args.eval_skill or args.eval_vae:
                skill_traj, skill_info = eval_skills(args, env, agent, epoch)

            # Plan
            if args.eval_plan and epoch % (args.il_eval_freq*2) == 0:
...
if __name__ == '__main__':
    ...
    arg_vv = vars(args)
    log_dir = 'data/pasta_plan'
    exp_name = '0001_test_pasta_planonly'
    arg_vv['wandb_group'] = exp_name
